...
class LemburController extends Controller
{
    ...
    public function masuk(Request $request)
    {
        ...
        if($request["jarak_masuk"] > $radius) {
            ...
            $fileName = 'foto_jam_masuk_lembur/' . date('Y-m-d_H-i-s') . '.png';
    ...
    public function pulang(Request $request, $id)
    {
        ...
        if($request["jarak_keluar"] > $radius) {
            ...
            $image_base64 = base64_decode($image_parts[1]);
            $fileName = 'foto_jam_keluar_lembur/' . date('Y-m-d_H-i-s') . '.png';
            ...
            $lembur->info_lembur = $request->input('info_lembur');
    ...
    {
        // Pastikan tanggal mulai dan akhir tersedia
        $request->validate([
            'mulai' => 'required|date',
            'akhir' => 'required|date',
        ]);

        // Ambil data lembur berdasarkan filter tanggal
        $data_lembur = Lembur::whereBetween('tanggal', [$request->mulai, $request->akhir])->get();

        // Tambahkan logika untuk menghitung kategori lembur
        foreach ($data_lembur as $dl) {
            $dl->kategori_lembur = $this->determineKategoriLembur($dl->jam_masuk, $dl->jam_keluar, $dl->tanggal);
        }

        return Excel::download(new LemburExport($data_lembur), 'lembur.xlsx');
    }

    private function determineKategoriLembur($jam_masuk, $jam_keluar)
    {
        $jam_masuk_time = strtotime($jam_masuk);
        $jam_keluar_time = strtotime($jam_keluar);

        if ($jam_masuk_time >= strtotime('18:30') && $jam_keluar_time <= strtotime('21:30')) {
            return 'Lembur 1 (18:30 - 21:30)';
        } elseif ($jam_masuk_time >= strtotime('21:30') && $jam_keluar_time <= strtotime('06:00')) {
            return 'Lembur 2 (21:30 - 06:00)';
        } elseif ($jam_masuk_time >= strtotime('18:30') && $jam_keluar_time <= strtotime('06:00')) {
            return 'Lembur 1 & 2 (18:30 - 06:00)';
        } elseif ($jam_masuk_time >= strtotime('08:00') && $jam_keluar_time <= strtotime('12:00')) {
            return 'Lembur 1 Weekend (08:00 - 12:00)';
        } elseif ($jam_masuk_time >= strtotime('13:00') && $jam_keluar_time <= strtotime('17:00')) {
            return 'Lembur 2 Weekend (13:00 - 17:00)';
        } elseif ($jam_masuk_time >= strtotime('08:00') && $jam_keluar_time <= strtotime('23:59')) {
            return 'Lembur 1 & 2 Weekend (08:00 - 23:59)';
        } else {
            return 'Tidak Masuk Kategori Lembur';
        }
    }
